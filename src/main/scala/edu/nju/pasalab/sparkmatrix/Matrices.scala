...
trait Matrix extends Serializable {
  ...
  private[sparkmatrix] def toBreeze: BM[Double]

  /** Gets the (i, j)-th element. */
  private[sparkmatrix] def apply(i: Int, j: Int): Double = toBreeze(i, j)
...
class DenseMatrix(val numRows: Int, val numCols: Int, val values: Array[Double]) extends Matrix {
  ...
  private[sparkmatrix] override def toBreeze: BM[Double] = new BDM[Double](numRows, numCols, values)
}
